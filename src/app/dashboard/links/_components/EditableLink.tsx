...
export default function EditableLink({ formData,idx }: any) {
    ...
    //console.log(formData.platform)
     const remove = useLinkStore((state) => state.removeLink)
...
return (
        <>
        {isDialogOpen&& (<div className="p-5"><LinkDeleteDialog handleRemoveLink={handleRemoveLink} isDialogOpen={isDialogOpen} setIsDialogOpen={setIsDialogOpen}/></div>)}
            <Form {...form} >
                <form onSubmit={form.handleSubmit(onSubmit)} className="w-full space-y-6 py-4 bg-light-grey rounded-md p-4">
                    <div className="flex justify-between mb-0" ><p className="text-[#333]/80">Link #{idx+1}</p>
                                    <button onClick={handleDialogClick} type='button'>Remove</button></div>
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                >
                                    ...
                                    <SelectContent >
                                        {PLATFORMS.map((platform)=>(<SelectItem value={platform.toString()} key={uuidv4()} ><span className="flex gap-2 items-center">{generatePlatformIcon(platform)} <p>{platform}</p></span></SelectItem>))}
